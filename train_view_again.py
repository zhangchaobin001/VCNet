...
import yaml
from AugFolder import AugFolder, MyFolder, ViewFolder
...
parser.add_argument('--part', action='store_true', help='use ft_net_part')
...
if opt.h == opt.w:
    transform_train_list = [
        ...
        #transforms.RandomResizedCrop(size=opt.inputsize),  # Image.BICUBIC)
    ...
    transform_train_list = [
        ...
        transforms.Pad(10),
        # transforms.RandomCrop((256,256)),
        transforms.RandomCrop(size=(opt.h, opt.w)),  # Image.BICUBIC)
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ]
...
if not opt.autoaug:
    ...
    #                                      data_transforms['train']) #(imgs, pid)
    image_datasets['train'] = ViewFolder(os.path.join(data_dir, 'train' + train_all),
...
if opt.balance:
    ...
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize, shuffle=True, num_workers=8) for x in ['train']}
    # dataloaders = {x: torch.utils.data.DataLoader(Preprocess(image_datasets[x]), batch_size=opt.batchsize, shuffle=True, num_workers=8, pin_memory=True) for x in['train']}
    ...
    for epoch in range(num_epochs - start_epoch):
        ...
        for phase in ['train']:
            ...
            for data in dataloaders[phase]:
                ...
                # get the inputs
                if opt.autoaug:
                    inputs, inputs2, labels, camid, view = data
                    view = np.array(view).astype(int)
                ...
                if use_gpu:
                    ...
                    view = Variable(view.cuda().detach())
                else:
                    inputs, view = Variable(inputs), Variable(view)
                ...
                running_corrects += float(torch.sum(preds == view.data))
            ...
            # deep copy the model
            if len(opt.gpu_ids) > 1 and epoch % 10 == 9:
...
if not opt.resume:
    ...
    if opt.use_dense:
        ...
        model = ft_net(3, opt.droprate, opt.stride, circle = False, ibn=False)
...
if opt.init_name != 'imagenet':
    ...
    if opt.use_dense:
        ...
        model = ft_net(3, opt.droprate, opt.stride, circle = False, ibn=opt.ibn)
...
if not opt.resume:
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
    # record every run
    copyfile('./train_view_again.py', dir_name + '/train.py')
...
print(model)
#model = ft_net(3, opt.droprate, opt.stride, circle = False, ibn=opt.ibn)
view_path = os.path.join('../mqveri/outputs', "views3", "net_last.pth")
