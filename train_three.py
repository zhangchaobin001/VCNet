...
from model import ft_net, ft_net_ours, ft_net_dense, ft_net_EF4, ft_net_EF5, ft_net_EF6, ft_net_IR, ft_net_NAS, \
    ft_net_SE, ft_net_DSE, PCB, CPB, ft_net_angle, ft_net_arc, three_select,qian_aware,ce_aware,hou_aware
...
parser.add_argument('--data_dir', default='../mqveri/train124', type=str, help='training dir path')
...
def train_model(model,model_backbone, model_qian, model_hou, model_ce, criterion, optimizer, scheduler, start_epoch=0, num_epochs=25):
    since = time.time()
        ...
        for phase in ['train']:
            ...
            for data in dataloaders[phase]:
                ...
                if phase == 'val':
                    ...
                    f = model_backbone(inputs)
                    _, f_hou = model_hou(f)
                    _, f_ce = model_ce(f)
                    _, f_qian = model_qian(f)
                    outputs, feature = model(f, f_qian, f_hou, f_ce)
...
exp_lr_scheduler = lr_scheduler.MultiStepLR(optimizer_ft, milestones=[100, 150, 180], gamma=0.1)
...
qian_path = os.path.join('../mqveri/outputs', "qian_aware", "net_060.pth")
...
hou_path = os.path.join('../mqveri/outputs', "hou_aware", "net_060.pth")
...
ce_path = os.path.join('../mqveri/outputs', "ce_aware", "net_060.pth")
...
if not opt.resume:
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
    # record every run
    copyfile('./train_three.py', dir_name + '/train.py')
...
print(model)
model = train_model(model, model_backbone, model_qian, model_hou, model_ce, criterion, optimizer_ft, exp_lr_scheduler,
