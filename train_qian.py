...
parser.add_argument('--data_dir', default='../mqveri/hou', type=str, help='training dir path')
...
def train_model(model, model_backbone, criterion, optimizer, scheduler, start_epoch=0, num_epochs=25):
    since = time.time()
        ...
        for phase in ['train']:
            ...
            for data in dataloaders[phase]:
                ...
                if phase == 'val':
                    ...
                    f_qian = model_backbone(inputs)
                    outputs, _ = model(f_qian)
...
if not opt.resume:
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
    # record every run
    copyfile('./train_qian.py', dir_name + '/train.py')
...
print(model)
model = train_model(model, model_backbone, criterion, optimizer_ft, exp_lr_scheduler,
