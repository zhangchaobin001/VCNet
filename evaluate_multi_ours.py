...
def evaluate(qf1, qf2, qf3, qv1, qv2, qv3, ql, qc1, qc2, qc3, gf, gv, gl, gc):
    ...
    #score = copy.deepcopy(score4)
    '''
    for i, value in enumerate(index_max):
        if value == 0 and qc1 != 1000:
            #if score1[i] > score2[i] + score3[i]:
            score[i] = 0.4 * score1[i] + 0.3 * score2[i] + 0.3 * score3[i]

        elif value == 1 and qc2 != 1000:
            #if score2[i] > score1[i] + score3[i]:
            #score2[i] = 0.8 * score2[i]
            score[i] = 0.4 * score2[i] + 0.3 * score1[i] + 0.3 * score3[i]

        elif value == 2 and qc3 != 1000:
            #if score3[i] > score2[i] + score1[i]:
            #score3[i] = 0.8 * score3[i]
            score[i] = 0.4 * score3[i] + 0.3 * score2[i] + 0.3 * score1[i]
        else:
    ...
    # index = index[0:2000]
    # good index
    query_index = np.argwhere(gl == ql) #返回的是索引
...
def compute_mAP(index, good_index, junk_index, gc, gv):
    ...
    ngood = len(good_index)   #good_index 代表真值
    ...
    #index_cam = index_map[mask_cam]

    rows_good = np.argwhere(mask == True)  #对应mask的索引
...
for i in range(0, len(query_label), 3):
    ...
    #pdb.set_trace()
    #if i%3==0:
    ap_tmp, CMC_tmp, inp_tmp, cp_tmp = evaluate(query_feature[i], query_feature[i + 1], query_feature[i + 2], query_view[i],
