...
import parser
from model import ft_net, VanillaVAE, LogCoshVAE, ft_net_ours, ft_net_dense, ft_net_EF4, ft_net_EF5, ft_net_EF6, \
    ft_net_IR, ft_net_NAS, ft_net_SE, ft_net_DSE, PCB, CPB, ft_net_angle, ft_net_arc, WAE_MMD, three_select, qian_aware, ce_aware, hou_aware

from cal_model import Baseline, resnet_model
...
def save_network(network, dirname, epoch_label):
    ...
    save_path = os.path.join('../mqveri/outputs',dirname,save_filename)
...
def load_network(name, opt):
    ...
    if opt.use_dense:
        ...
        #model = Baseline(opt.nclasses, 1, '/home/cbzhang4/.cache/torch/checkpoints/resnet50-19c8e357.pth', True)
        #model = ft_net(opt.nclasses, droprate=0, stride=2, ibn=opt.ibn)
        #model = three_select(124)
        #model = resnet_model(remove_downsample=True)
        model = ft_net_ours(opt.nclasses, droprate=0, stride=opt.stride, ibn=opt.ibn)
    ...
    if isinstance(epoch, int):
        ...
        #save_filename = 'net_%03d.pth' % 30
    else:
        save_filename = 'net_%s.pth'% epoch
        #save_filename = 'net_%s.pth' % 30
